...
let apiKey = "8ae61aa80a8d5ad912ec629b5bd69b7a"
...
class FlickrPhoto : Equatable {
    ...
    func loadLargeImage(completion: (flickrPhoto:FlickrPhoto, error: NSError?) -> Void) {
        ...
        let task = NSURLSession.sharedSession().dataTaskWithRequest(loadRequest) { data, response, error in
            if error != nil {
...
class Flickr {
    ...
    func searchFlickrForTerm(searchTerm: String, completion : (results: FlickrSearchResults?, error : NSError?) -> Void){
        ...
        let task = NSURLSession.sharedSession().dataTaskWithRequest(searchRequest) { data, response, error in
            if error != nil {
            ...
            do {
                resultsDictionary = try NSJSONSerialization.JSONObjectWithData(data!, options: NSJSONReadingOptions.AllowFragments) as? NSDictionary
            } catch {
    ...
    private func flickrSearchURLForSearchTerm(searchTerm:String) -> NSURL {
        ...
        let escapedTerm = searchTerm.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.alphanumericCharacterSet())!
